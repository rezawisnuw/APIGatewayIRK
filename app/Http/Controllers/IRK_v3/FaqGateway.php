...
class FaqGateway extends Controller
{
    ...
    public function get(Request $request)
    {
        try {
            ...
            if ($decode_signature->result == 'Match') {
                $response = $this->helper->Client('toverify_gcp')->request('POST', $this->base . '/faq/get', [
                    'json' => [
                        'data' => $request->all()
                    ]
                ]);
    ...
    public function post(Request $request)
    {
        try {
            ...
            if ($decode_signature->result == 'Match') {
                $response = $this->helper->Client('toverify_gcp')->request('POST', $this->base . '/faq/post', [
                    'json' => [
                        'data' => $request->all()
                    ]
                ]);
