...
class UtilityGateway extends Controller
{
    public function LoginESS(Request $request){
        ...
        try {
            ...
            return response()->json(['result' => $th->getMessage(), 'data' => [], 'message' => 'Error in CatchDev', 'status' => 0, 'statuscode' => $th->getCode()]);
        }
    }
				...
				try {
					...
					foreach($result as $key=>$value){
						...
						if(isset($value->NIK)){
							...
							$client = new Client();
							$response = $client->post(
								'http://'.config('app.URL_GCP_LARAVEL_SERVICELB').'dev/profile/get',
								[
									RequestOptions::JSON =>[
										'data' => $object
									]
