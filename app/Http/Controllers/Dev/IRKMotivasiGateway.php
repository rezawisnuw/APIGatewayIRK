            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                ...
                if(!empty($result->data)){
                    ...
                    foreach($result->data as $key=>$value){
                        ...
                        if(!empty($value->picture) && str_contains($value->picture,'Dev/Ceritakita/Motivasi/') && in_array(explode('.',$value->picture)[1], $format)){
        ...
        try {
            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                if(!empty($request->photo)){
                    $response = (new self)->client('toverify_gcp')->request('POST', 'dev/motivasi/post', [
                        'multipart'=>[
                            [
                                'name' => 'data',
                    ...
                    if(!empty($result->data)){
                        ...
                        $newresponse = $newclient->post(
                            'http://'.config('app.URL_GCP_LARAVEL_SERVICELB').'dev/motivasi/get',
                            [
                                RequestOptions::JSON => 
                                [
                                    'data' => [
