...
class UtilityGateway extends Controller
{
    public function LoginESS(Request $request){
        ...
        try {
            ...
            return response()->json(['result' => $th->getMessage(), 'data' => [], 'message' => 'Error in CatchLive', 'status' => 0, 'statuscode' => $th->getCode()]);
        }
    }
			...
			if($escapestring_token == $param['param']['nik']){ 
				try {
					...
					foreach($result as $key=>$value){
						...
						$newjson->JENIS_KELAMIN = $value->NIK == '000001' ? 'PRIA' : $value->JENIS_KELAMIN;
