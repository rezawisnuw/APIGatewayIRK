...
class IRKCeritaKitaGateway extends Controller
{
    ...
    public function get(Request $request){
        try {
            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                $response = (new self)->client('toverify_gcp')->request('POST', 'live/ceritakita/get', [
                    'json'=>[
                        'data' => $request->all()
                    ]
                ]);
                ...
                if(!empty($result->data)){                    
                    ...
                    foreach($result->data as $key=>$value){
                        ...
                        if(!empty($value->picture) && str_contains($value->picture,'Live/Ceritakita/') && in_array(explode('.',$value->picture)[1], $format)){
                    ...
                    $newresponse = $newclient->post(
                        'http://'.config('app.URL_GCP_LARAVEL_SERVICELB').'live/ceritakita/get',
                        [
                            RequestOptions::JSON => 
                            [
                                'data' => [
