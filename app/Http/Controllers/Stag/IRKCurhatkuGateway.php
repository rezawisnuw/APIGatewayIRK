    ...
    public function get(Request $request){
        try {
            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                $response = (new self)->client('toverify_gcp')->request('POST', 'stag/curhatku/get', [
                    'json'=>[
                        'data' => $request->all()
                    ]
                ]);
                ...
                if(!empty($result->data)){
                    ...
                    foreach($result->data as $key=>$value){
                        ...
                        if(!empty($value->picture) && str_contains($value->picture,'Stag/Ceritakita/Curhatku/') && in_array(explode('.',$value->picture)[1], $format)){
                ...
                if(!empty($request->gambar)){
                    ...
                    if(!empty($result->data)){
                        ...
                        $newresponse = $newclient->post(
                            'http://'.config('app.URL_GCP_LARAVEL_SERVICELB').'stag/curhatku/get',
                            [
                                RequestOptions::JSON => 
                                [
                                    'data' => [
