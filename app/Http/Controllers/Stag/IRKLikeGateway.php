...
class IRKLikeGateway extends Controller
{
    ...
    {
        if(!empty($data)){
            if(str_contains($data,'Success') || $data == true || $data != null){
    ...
    public function get(Request $request){
        try {
            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                $response = (new self)->client('toverify_gcp')->request('POST', 'stag/like/get', [
                    'json'=>[
                        'data' => $request->all()
                    ]
                ]);
    ...
    public function post(Request $request){
        try {
            ...
            if($this->userValid($request)->getData()->result == 'Match'){
                $response = (new self)->client('toverify_gcp')->request('POST', 'stag/like/post', [
                    'json'=>[
                        'data' => $request->all()
                    ]
                ]);
